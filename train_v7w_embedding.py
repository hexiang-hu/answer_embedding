...
parser.add_argument('--max_negative_answer', default=50000, type=float)
parser.add_argument('--answer_batch_size', default=3000, type=float)
...
def test(context_net, answer_net, loader, tracker, args, prefix='', epoch=0):
  ...
  cnt = 0
  for v, q, _, _, a_ids, idx, q_len in tq:
...
def main(args):
  if args.name is None:
    ...
    name = args.context_embedding+"_"+args.answer_embedding+"_v7w_batch_softmax_embedding_"+datetime.now().strftime("%Y-%m-%d_%H:%M:%S")
  else:
  ...
  if args.pretrained_model is not None:
    ...
    print('* Visual7W Val Accuracy: {}'.format(torch.cat(accs).mean()))
    accs = test(context_net, answer_net, test_loader, tracker, args, prefix='test', epoch=-1)
    print('* Visual7W Test Accuracy: {}'.format(torch.cat(accs).mean()))

    test_loader.dataset.set_v7w_server(True)
    accs = test(context_net, answer_net, test_loader, tracker, args, prefix='test', epoch=-1)
    print('* V7W Test Accuracy: {}'.format(torch.cat(accs).mean()))
    test_loader.dataset.set_v7w_server(False)
  ...
  for i in range(cfg.TRAIN.epochs):
    ...
    print('* Visual7W Val Accuracy:    {}'.format(torch.cat(r).mean()))
    val_acc = 0.5 * torch.mean( torch.cat(r, dim=0) )

    val_loader.dataset.set_v7w_server(True)
    r = test(context_net, answer_net, val_loader, tracker, args, prefix='val', epoch=i)
    print('* Visual7W Val Accuracy:    {}'.format(torch.cat(r).mean()))
    val_loader.dataset.set_v7w_server(False)

    val_acc += 0.5 * torch.mean( torch.cat(r, dim=0) )
    _eval.append({ 'accuracies': r })

    if best_val_acc < val_acc:
      accs = test(context_net, answer_net, test_loader, tracker, args, prefix='test', epoch=i)
      print('* Visual7W Test Accuracy: {}'.format(torch.cat(accs).mean()))
